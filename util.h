...
public:
    ...
    Vector3D reflect_about(const Vector3D& other) const;
    Vector3D rotate_about(const Vector3D& other, double angle) const;
    Vector3D noncolinear() const; 
    Vector3D orthogonal() const; 
    double cos_ang(const Vector3D& other) const;
    double sin_ang(const Vector3D& other) const;
    double ang(const Vector3D& other) const;
...
struct Material {
	Material( Colour ambient, Colour diffuse, Colour specular, double exp , double rfr_i, double rfr_c, double rfl_c, double gl_n, double gl_d) :
		ambient(ambient), diffuse(diffuse), specular(specular), 
		specular_exp(exp), refraction_index(rfr_i), refraction_coefficient(rfr_c), 
        reflection_coefficient(rfl_c), glossy_num(gl_n), glossy_deviation(gl_d) {}
    ...
    // Reflection number of rays for glossy reflection
    double glossy_num;
    // Reflection deviation of rays for glossy reflection
    double glossy_deviation;
};
...
struct Ray3D {
	...
	Ray3D( const Point3D p, const Vector3D v ) : origin(p), dir(v) {
		intersection.none = true;
	}
    Ray3D(const Ray3D& other);
    Point3D operator[](double t);
    Ray3D& operator =(const Ray3D& other);
    Ray3D spawn_reflection_ray();
    Ray3D spawn_refraction_ray(double theta1, double theta2);
