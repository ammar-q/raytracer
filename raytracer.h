...
public:
    ...
    void anti_aliased_render( int width, int height, Point3D eye, Vector3D view, 
...
private:
    ...
    // Collect 
    double lightCollectionTraversal( SceneDagNode* node, Ray3D& ray );
    double lightCollectionTraversal( SceneDagNode* node, Ray3D& ray, const Matrix4x4& modelToWorld, const Matrix4x4& worldToModel );
    
    // Check if the path between two points is blocked
    double pathUnblocked( const Point3D& source, const Point3D& dest );
    
    // Refraction Index at point
    double computeRefractionIndex( SceneDagNode* node, Point3D& point);
    double computeRefractionIndex( SceneDagNode* node, Point3D& point, const Matrix4x4& modelToWorld, const Matrix4x4& worldToModel );
